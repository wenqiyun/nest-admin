...
export class PostService {
  ...
  async create (dto: CreatePostDto): Promise<ResultData> {
    const existing = await this.postRepo.findOne({ code: dto.code, name: dto.name })
  ...
  async update (dto: UpdatePostDto): Promise<ResultData> {
    const existing = await this.postRepo.findOne(dto.id)
  ...
  async delete (id: string): Promise<ResultData> {
    const existing = await this.postRepo.findOne(id)
